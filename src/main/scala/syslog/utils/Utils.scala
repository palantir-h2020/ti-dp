...
class Utils(appConfigFile: String) extends Serializable {
  /**
  ...
  def loadProperties(propertiesFilename: String): Properties = {
    ...
    logger.dev("Loading properties from file " + propertiesFilename + ".")
    ...
    try {
      ...
      case e: IOException =>
        logger.dev("Cannot initialize app properties from file " + propertiesFilename + " Cause: " + e.getMessage)
        logger.dev("Loaded the default values.")
        appProps.setProperty("kafka.broker.ip", "10.101.41.255")
        appProps.setProperty("kafka.broker.port", Integer.toString(9092))
        appProps.setProperty("kafka.topic.input", "syslog-raw-dev")
        appProps.setProperty("kafka.topic.output", "syslog-preprocessed")
        appProps.setProperty("spark.master.url", "local[*]")
        appProps.setProperty("spark.streaming.interval.seconds", Integer.toString(5))
        appProps.setProperty("spark.app.name", "Palantir Preprocessing Syslog Application")
    }
...
//pipeline.write.overwrite().save("tf-idf-pipeline-model")
//
//val model = pipeline.fit(datasetDf)
//model.write.overwrite().save("tf-idf-model")
//
//val tfidfDf = model.transform(datasetDf)
//
////    val sameModel = PipelineModel.load("models/tf-idf-model")